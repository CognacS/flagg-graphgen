...
def main():
    ...
    seeds_opts = [[f'load_ckp={seed}', f"seed={seed}"] for seed in seeds]
    model = [f"+preset/ifh/enz={block}" for block in blocks]
    outputs = [f"outputs/enz_traces/enz_{seed}_{block}.txt" for seed in seeds for block in blocks]
    ...
    cmds = [command + [mdl] + common_commands + seed for seed in seeds_opts for mdl in model]
    
    all_results = run_parallel(cmds, outputs, max_workers=2)
