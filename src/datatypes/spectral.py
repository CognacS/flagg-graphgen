...
def get_eigenvalues_features(eigenvalues, node_mask, num_nodes_per_sample, k=5):
    """
    ...
    eigenvalues = eigenvalues * node_mask + 2 * torch.ones_like(num_nodes_per_sample).unsqueeze(-1) * (~node_mask)

    to_extend = max(n_connected_components) + k - n
    if to_extend > 0:
        padding = torch.full((bs, to_extend), 2).type_as(eigenvalues)
        eigenvalues = torch.hstack((eigenvalues, padding))
...
def get_eigenfeatures_from_adjmat(adjmat: Tensor, node_mask: Tensor, mode: str='all', topk_eigvals: int=5, topk_eigvecs: int=2, float64: bool=False):

    if adjmat.numel() == 0:
        n_connected_comp = adjmat.new_zeros((adjmat.shape[0], 1))
        batch_eigenvalues = adjmat.new_full((adjmat.shape[0], topk_eigvals), 2)
        nonlcc_indicator = adjmat.new_zeros((adjmat.shape[0], adjmat.shape[1], 1))
        k_lowest_eigenvector = adjmat.new_zeros((adjmat.shape[0], adjmat.shape[1], topk_eigvecs))
        return n_connected_comp, batch_eigenvalues, nonlcc_indicator, k_lowest_eigenvector

    mask = node_mask
    if adjmat.ndim == 4:
        A = adjmat[..., 1:].sum(dim=-1).float()
    else:
        A = adjmat.float()
    ...
    if mode == 'eigenvalues':
        ...
        num_nodes = torch.sum(mask, dim=1, keepdim=True)
        num_nodes[num_nodes == 0] = 1
        eigvals = eigvals.type_as(A) / num_nodes

        n_connected_comp, batch_eigenvalues = get_eigenvalues_features(
            eigenvalues=eigvals,
            node_mask=node_mask,
            num_nodes_per_sample=num_nodes.squeeze(-1),
            k=topk_eigvals
        )
        return n_connected_comp.type_as(A), batch_eigenvalues.type_as(A), None, None

    elif mode == 'all':
        try: # try to use cuSolver as-is
            if L.shape[-1] > 1000: # this will be fixed with CUDA 12.1, and the conversion won't be necessary
                eigvals, eigvectors = torch.linalg.eigh(L.to(torch.float64))
                eigvals = eigvals.to(L.dtype)
                eigvectors = eigvectors.to(L.dtype)
            else:
                eigvals, eigvectors = torch.linalg.eigh(L)
        except (LinAlgError, RuntimeError) as e: # if it fails, use magma, which is almost assured to work
            print('LinAlgError raised, temporarily setting magma as the preferred linalg library')
        ...
        nonlcc_indicator, k_lowest_eigenvector = get_eigenvectors_features(vectors=eigvectors,
                                                                            ...
                                                                            k=topk_eigvecs)
        return n_connected_comp.type_as(A), batch_eigenvalues.type_as(A), nonlcc_indicator.type_as(A), k_lowest_eigenvector.type_as(A)
    else:
        raise NotImplementedError(f"Mode {mode} is not implemented")
    


def get_node_cycle_features_from_adjmat(adjmat: Tensor, node_mask: Tensor):
    ...
    x_cycles, y_cycles = kcycles.k_cycles(adj_matrix=A)   # (bs, n_cycles)
    x_cycles = x_cycles.type_as(A) * node_mask.unsqueeze(-1)
    # Avoid large values when the graph is dense
    # x_cycles = x_cycles / 10
    # y_cycles = y_cycles / 10
    #x_cycles[x_cycles > 100] = 100
    #y_cycles[y_cycles > 100] = 100
