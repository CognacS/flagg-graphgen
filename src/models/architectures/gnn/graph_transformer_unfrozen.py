...
class UnfrozenXEyTransformerLayer(nn.Module):
    ...
    def forward(
            ...
            E_bbba: Tensor,
            y: Tensor,
            node_mask_ba: Tensor,   # new + old
            node_mask_b: Tensor,    # only new
            node_mask_a: Tensor,    # only old
            edge_mask_bbba: Optional[Tensor]=None,
        ...
        # X = norm(X + FFN(X))
        newX = self.ffnX(X_ba)
        X_ba = self.normX2(X_ba + newX)

        # E = norm(E + FFN(E))
        newE = self.ffnE(E_bbba)
        E_bbba = self.normE2(E_bbba + newE)

        # y = norm(y + FFN(y))
        new_y = self.ffny(y)
        y = self.norm_y2(y + new_y)

        return X_ba, E_bbba, y
...
class XEyBlockAttention(nn.Module):
    ...
    def __init__(
        ...
        self.bbba_attn = OuterAttention(dx, de, n_head) # B to B and A (self-attention)
    ...
    def forward(
            ...
            node_mask_b: Tensor,    # only new
            node_mask_a: Tensor,    # only old
            E_bbba: Tensor,         # new + old
            y: Tensor,
            edge_mask_bbba: Optional[Tensor]=None,    # new + old
            edge_mask_ba: Optional[Tensor]=None       # only old
        ) -> tuple[Tensor, Tensor, Tensor]:
        ...
        # E_bbba is rectangular, where the first dimension is that of the new block
        # while the second is the sum of the new and old blocks
        # to get the number of nodes from the new block, just subtract
        bs, nb, nb_na, _ = E_bbba.shape
        na = nb_na - nb

        #######################  FAKE NODES MASKS SETUP  #######################

        # unsqueeze to enable masking (dot product only if same number of dims)
        xba_mask = node_mask_ba             # (bs, nb+na, 1)
        eba_mask = edge_mask_bbba           # (bs, nb, nb+na, 1)

        # get new and old nodes from splitting
        X_b, X_a = X_ba.split([nb, na], dim=1)          # (bs, nb, dx), (bs, na, dx)

        # get interlinking edges
        E_ba = E_bbba[:, :, nb:]                        # (bs, nb, na, de)
        E_ab = E_ba.transpose(1, 2)                     # (bs, nk, nq, de)
        edge_mask_ab = edge_mask_ba.transpose(1, 2)	    # (bs, nk, nq, 1)

        # B pays attention to itself and A
        V_b: Tensor     # (bs, nb, dx)
        A_bbba: Tensor  # (bs, nb, nb+na, de)
        V_b, A_bbba = self.bbba_attn(
            Xq=X_b, Xk=X_ba, E=E_bbba,
            node_mask_q=node_mask_b, node_mask_k=node_mask_ba, edge_mask=edge_mask_bbba
        )
        A_bb, A_ba = A_bbba.split([nb, na], dim=-2)
        
        # A pays attention only to B
        V_a: Tensor     # (bs, na, dx)
        A_ab: Tensor    # (bs, na, nb, de)
        V_a, A_ab = self.ab_attn(
        ...
        # concatenate values and attention back
        V_ba = torch.cat([V_b, V_a], dim=1)             # (bs, nb+na, dx)
        A_bbba = torch.cat([A_self, A_cross], dim=2)    # (bs, nb, nb+na, de)
        ...
        newE = ye1 + (ye2 + 1) * A_bbba

        # Output E
        newE = self.e_out(newE) * eba_mask					# (bs, nb, nb+ba, de)
        ...
        # Output X
        newX = self.x_out(newX) * xba_mask      # (bs, nb+na, dx)
        ...
        e_y = self.reduce_e(E_bbba, eba_mask)		# (bs, dy)
        x_y = self.reduce_x(X_ba, xba_mask)		# (bs, dy)
...
class GraphTransformerUnfrozen(nn.Module):
    """
    ...
    def forward(
        ...
        if using_ext:
            ...
            X = torch.cat([X, ext_X], dim=1)
            node_mask_ba = torch.cat([node_mask, ext_node_mask], dim=1)

            ext_node_mask = torch.cat([node_mask, ext_node_mask], dim=1)
            edge_mask_ba = ext_edges.edge_mask.unsqueeze(-1)
            edge_mask_bbba = torch.cat([edge_mask, edge_mask_ba], dim=2)
            E = torch.cat([E, ext_E], dim=2)

            # for each layer use the only provided set of external encoded nodes
            for layer in self.tf_layers:
                X, E, y = layer(
                    X, E, y, node_mask_ba=node_mask_ba,
                    node_mask_b=node_mask, node_mask_a=ext_node_mask,
                    edge_mask_bbba=edge_mask_bbba, edge_mask_ba=edge_mask_ba
                )


            # split back X and E
            X, ext_X = X.split([nq, nk], dim=1)
