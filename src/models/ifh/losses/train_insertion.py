...
class DistributionInsertionLoss(nn.Module):
    """
    ...
    def forward(
        ...
        if ret_log:
            to_log = {
                labels.INSERTION_LOSS_KL: kl_div.detach(),
            }
...
class RegressionInsertionLoss(nn.Module):
    """
    ...
    def forward(
        ...
        if ret_log:
            to_log = {
                labels.INSERTION_LOSS_MSE: support_term.detach(),
            }
            return total_loss, to_log
...
class CategoricalInsertionLoss(nn.Module):
    ...
    def forward(
        ...
        if ret_log:
            to_log = {
                labels.INSERTION_LOSS_CE: ce.detach(),
            }
            return total_loss, to_log
