...
from src.evaluation.utils.distributions import compute_mmd, gaussian_emd, gaussian, emd, gaussian_tv, disc
...
def nodes_stats(graph_ref_list, graph_pred_list, compute_emd=False, **kwargs):
    num_nodes_test = [len(gg.nodes) for gg in graph_ref_list]  # shape B X 1
    num_nodes_gen = [len(aa.nodes) for aa in graph_pred_list]
    mmd_nodes = compute_mmd([np.bincount(num_nodes_test[:len(graph_pred_list)])], [np.bincount(num_nodes_gen)],
                                        kernel=gaussian_tv)
...
def list_to_nx(graph_list) -> List[nx.Graph]:
    networkx_graphs = []
    for i, graph in enumerate(graph_list):
        networkx_graphs.append(to_networkx(graph, node_attrs=None, edge_attrs=None, to_undirected=True,
                                           remove_self_loops=True))
    return networkx_graphs
